...
$('#facebook').on("click", function() {
 sharefb('http://mabu.newscloud.sbs.co.kr/201810fairtrade/3');
});

$('#twitter').on("click", function() {
 sharetwit('http://mabu.newscloud.sbs.co.kr/201810fairtrade/3', 'SBS마부작침: 공정이란 무엇인가 - 공정위 과징금의 공정(公正)을 묻다');
});
...
var days = ["SK", "삼성", "LG", "현대자동차", "롯데", "금호아시아나", "LS", "두산", "한화", "대림", "효성", "CJ", 
			"GS", "포스코", "kt", "한진", "현대중공업", "농협", "KCC", "신세계", "현대백화점", "OCI", "교보생명보험", 
			"부영", "S-Oil", "미래에셋", "KT&G", "영풍", "한국투자금융"],
...
var width = Math.max(Math.min(window.innerWidth, 1280), 670) - margin.left - margin.right - 20,
	...
	height = gridSize * (days.length+2) /2.5;
...
var dayLabels = svg.selectAll(".dayLabel")
    ...
    .attr("x", 0)
    .attr("y", function (d, i) { return (i-0.6)* gridSize /2.375; })
...
var heatMap = svg.selectAll(".hour")
    ...
    .attr("y", function(d) { return (d.day - 1) * gridSize /2.375; })
...
var legendsvg = svg.append("g")
	...
	.attr("transform", "translate(" + (width/2) + "," + (gridSize * days.length+100)/2.375 + ")");
